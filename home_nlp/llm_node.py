...
This node subscribes to transcribed speech from the "llm_input" topic
...
Responses generated by the LLM are published to the "llm_output" topic
...
class LargeLanguageModelNode(LifecycleNode):
    ...
    def llm_input_cb(self, msg: String):
        self.get_logger().debug("Received input")
        try:
            self.input_buffer.put_nowait(msg.data)
        except queue.Full:
            self.get_logger().warn("Input buffer full, dropping chunk.")

    def timer_cb(self):
        if self.input_buffer.empty():
            return

        user_input = self.input_buffer.get_nowait()
